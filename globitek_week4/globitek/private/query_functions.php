<?php
  ...
  function validate_country($country, $errors=array()) {
    ...
    } elseif (!has_valid_input_format($country['name'])) {
      $errors[] = "Input can only contain letters, numbers, and underscores.";
    }
    ...
    } elseif (!has_valid_input_format($country['code'])) {
      $errors[] = "Input can only contain letters, numbers, and underscores.";
    }
  ...
  function validate_state($state, $errors=array()) {
    if (is_blank($state['name'])) {
      ...
      $errors[] = "Name must be between 2 and 255 characters.";
    } elseif (!has_valid_input_format($state['name'])) {
      $errors[] = "Input can only contain letters, numbers, and underscores.";
    }
    ...
    if (is_blank($state['code'])) {
      ...
      $errors[] = "Code must be between 2 and 255 characters.";
    } elseif (!has_valid_input_format($state['code'])) {
      $errors[] = "Input can only contain letters, numbers, and underscores.";
    }

    if (is_blank($state['country_id'])) {
      $errors[] = "Country ID cannot be blank.";
    } elseif (!has_valid_input_format($state['country_id'])) {
      $errors[] = "Input can only contain letters, numbers, and underscores.";
    }
  ...
  function validate_territory($territory, $errors=array()) {
    if (is_blank($territory['name'])) {
      ...
      $errors[] = "Name must be between 2 and 255 characters.";
    } elseif (!has_valid_input_format($territory['name'])) {
      $errors[] = "Input can only contain letters, numbers, and underscores.";
    }

    if (is_blank($territory['state_id'])) {
      $errors[] = "State ID cannot be blank.";
    } elseif (!has_valid_input_format($territory['state_id'])) {
      $errors[] = "Input can only contain letters, numbers, and underscores.";
    }

    if (is_blank($territory['position'])) {
      $errors[] = "Position cannot be blank.";
    } elseif (!has_valid_input_format($territory['position'])) {
      $errors[] = "Input can only contain letters, numbers, and underscores.";
    }
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    ...
    } elseif (!has_valid_input_format($salesperson['first_name'])) {
      $errors[] = "Input can only contain letters, numbers, and underscores.";
    }
    ...
    } elseif (!has_valid_input_format($salesperson['last_name'])) {
      $errors[] = "Input can only contain letters, numbers, and underscores.";
    }
